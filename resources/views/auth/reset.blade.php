  ...
  <div class="ring">
    ...
    <div class="login">
      ...
      <form method="POST" action="{{ route('password.update') }}" class="form" novalidate>
        ...
        {{-- Nueva contraseña (error arriba) --}}
        @if ($errors->has('password'))
          <div class="field-error">{{ $errors->first('password') }}</div>
        @endif
        <div class="inputBx has-toggle @error('password') invalid @enderror">
          <input id="password" type="password" name="password" placeholder="Nueva contraseña" required autocomplete="new-password">
  ...
  <script>
    (function(){
      document.querySelectorAll('.toggle-visibility').forEach(function(btn){
        ...
        if (eyeOff) eyeOff.style.display = 'none'; // estado inicial
